...
const getAllSendedRequestsController: fastifyController = async (request, reply) => {
    ...
    try {
        ...
        const { result, nextCursor, isTheLastPage } = await getPortionOfFriendshipRequestsList(bufferizedCursor, internalUser.id, 's')
